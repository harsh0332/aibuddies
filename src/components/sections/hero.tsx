  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center px-4 sm:px-6">
        <h1
          className={`kinetic text-center text-[28px] sm:text-[40px] md:text-[60px] leading-[1.1] font-light tracking-tight font-sora ${
      ...
      <div
        className={`intro-up absolute bottom-0 left-0 right-0 z-20 px-6 sm:px-8 md:px-12 pb-8 md:pb-12 transition-all duration-300 ${
          isArmed ? "intro-up--armed" : ""
        } ${isDone ? "intro-up--done" : ""}`}
      >
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between max-w-5xl mx-auto w-full">
          <div className="max-w-md text-center md:text-left">
            <p className="text-sm sm:text-base md:text-lg text-[#f6f6fd]/60 leading-relaxed font-sora">
              We help businesses stop doing things manually and start running on AI.
            </p>
            <div className="flex justify-center md:justify-start">
              <Magnetic>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="btn-shine inline-flex w-fit items-center gap-2 whitespace-nowrap transition-transform hover:scale-[1.03] mt-4 md:mt-5 text-sm font-semibold text-[#021018] px-6 py-3 rounded-full bg-[#43C2D8] shadow-[0_0_10px_0_rgba(43,160,220,0.55)] focus:outline-none focus:ring-2 focus:ring-[#f6f6fd]"
                >
                  Book a Free Call
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M5 12h14M13 6l6 6-6 6"/>
          ...
          <div className="flex justify-center md:justify-start gap-8 sm:gap-12">
            {[
              ...
              <div key={v} className="max-w-[120px] sm:max-w-[150px] flex flex-col justify-end text-center md:text-left">
                <div className="text-xl sm:text-[30px] font-bold text-[#f6f6fd] leading-none font-sora">{k}</div>
                <div className="text-[10px] md:text-[11px] text-[#f6f6fd]/60 mt-2 md:mt-3 font-mono uppercase tracking-wider leading-tight">{v}</div>
              </div>
            ))}
          </div>
        </div>
